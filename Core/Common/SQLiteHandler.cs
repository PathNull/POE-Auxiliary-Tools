...
namespace Core.SQLite
{
    public class SQLiteHandler
    {
        ...
        /// 数据库列表
        /// </summary>
        public static Dictionary<string, SQLiteHandler> DataBaceList = new Dictionary<string, SQLiteHandler>();

        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="filename">数据库文件名</param>
        public SQLiteHandler(string filename = null)
        {
            DataSource = filename;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public bool IsExist(string tableName,Dictionary<string,string> dic)
        {
            ...
            using (SQLiteConnection connection = GetSQLiteConnection())
            {
                ...
                {
                    sbr.Append($"WHERE {item.Key}='{item.Value}'");
                }
